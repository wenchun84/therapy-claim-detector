...
def index():
    if request.method == 'POST':
        ...
        if not text_input and (not file or file.filename == ''):
            return render_template('index.html', title='文章检测', active_tab='article', error='请输入文字或上传文件')
...
if text_input:
    ...
    if found_violations:
        ...
        if file:
            ...
            return render_template('result.html', title='檢測結果', active_tab='article', result=result, filename=filename)
